import { CreateMembershipTierInputV2 } from "~/server/membershipTier/types";
...
export default class CreateMembershipTierV2Command
  implements Command<SystemState, Services>
{
  private readonly input: CreateMembershipTierInputV2;
  ...
  async run(m: SystemState, r: Services): Promise<void> {
    ...
    const result = await r.membershipTier.createMembershipTierV2(
      this.clubId,
      this.input
    );
    this.membershipTierId = idAsNumber(result.createdEntityId);
    m.createMembershipTierV2(this.membershipTierId, this.clubId, this.input);
